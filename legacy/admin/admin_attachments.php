...
if ($mode == 'quota') {
    ...
    for ($i = 0, $iMax = count($rows); $i < $iMax; $i++) {
        ...
        $template->assign_block_vars('limit_row', [
...
if ($mode == 'quota' && $e_mode == 'view_quota') {
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        if ($rows[$i]['quota_type'] == QUOTA_UPLOAD_LIMIT) {
            $template->assign_block_vars('users_upload_row', [
                'USER_ID'  => $rows[$i]['user_id'],
                'USERNAME' => $rows[$i]['username'],
            ]);
        } elseif ($rows[$i]['quota_type'] == QUOTA_PM_LIMIT) {
            $template->assign_block_vars('users_pm_row', [
                'USER_ID'  => $rows[$i]['user_id'],
                'USERNAME' => $rows[$i]['username'],
            ]);
        }
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        if ($rows[$i]['quota_type'] == QUOTA_UPLOAD_LIMIT) {
            $template->assign_block_vars('groups_upload_row', [
                'GROUP_ID'  => $rows[$i]['group_id'],
                'GROUPNAME' => $rows[$i]['group_name'],
            ]);
        } elseif ($rows[$i]['quota_type'] == QUOTA_PM_LIMIT) {
            $template->assign_block_vars('groups_pm_row', [
                'GROUP_ID'  => $rows[$i]['group_id'],
                'GROUPNAME' => $rows[$i]['group_name'],
            ]);
        }
