...
<!-- IF GROUP_TYPE -->
<div class="pad_4">{L_GROUP_TYPE}: <b>{GROUP_TYPE}</b></div>
<!-- ENDIF -->

<!-- IF PAGINATION -->
<div class="pad_4">{PAGE_NUMBER}</div>
<!-- ENDIF -->
...
<form action="{S_GROUP_ACTION}" method="post">
    ...
    <table class="forumline pad_4">
        ...
        <tr>
            ...
            <td>
                <div class="post_wrap">{GROUP_SIGNATURE}</div>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><p>{GROUP_DETAILS}
                    ...
                    <input class="mainoption" type="submit" name="joingroup" value="{L_JOIN_A_GROUP}"
                           onclick="return confirm('{L_JOIN_A_GROUP}?');"/>
                    <!-- ENDIF -->
                    <!-- IF SHOW_UNSUBSCRIBE_CONTROLS -->
                    <input class="mainoption" type="submit" name="{CONTROL_NAME}" value="{L_UNSUBSCRIBE_GROUP}"
                           onclick="return confirm('{L_UNSUBSCRIBE_GROUP}?');"/>
                    <!-- ENDIF -->
                </p>
            </td>
        </tr>
...
<p class="nav"><a href="{U_GROUP_MEMBERS}" name="members">{L_GROUP_MEMBERS}</a><!-- IF RELEASE_GROUP -->&nbsp;::&nbsp;<a
        href="{U_GROUP_RELEASES}" name="releases">{L_GROUPS_RELEASES}</a><!-- ENDIF --></p>
        ...
        <tr>
            <td colspan="11" class="cat" style="padding: 2px 12px;">
                <p id="add_group_member" class="floatL">
                    ...
                    <input type="button" name="usersubmit" value="{L_FIND_USERNAME}" class="liteoption"
                           onclick="window.open('{U_SEARCH_USER}', '_bbsearch', IWP_US);return false;"/>
                </p>
                <p class="floatR" style="padding-top: 1px;">
                    <input type="submit" name="remove" value="{L_REMOVE_SELECTED}" class="mainoption"
                           onclick="return confirm('{L_REMOVE_SELECTED}?');"/>
                </p>
            </td>
        </tr>
        </tfoot>
        ...
        <tr>
            <td class="cat" colspan="9">
                <input type="submit" name="approve" value="{L_APPROVE_SELECTED}"
                       onclick="return confirm('{L_APPROVE_SELECTED}?');" class="mainoption"/>
                &nbsp;
                <input type="submit" name="deny" value="{L_DENY_SELECTED}"
                       onclick="return confirm('{L_DENY_SELECTED}?');" class="liteoption"/>
            </td>
        </tr>
        </tfoot>
    </table>
