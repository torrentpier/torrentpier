...
function bb_update_config($params, $table = BB_CONFIG)
{
    ...
    $updates = OLD_DB()->build_array('MULTI_INSERT', $updates);

    OLD_DB()->query("REPLACE INTO $table $updates");
...
function setup_style()
{
    ...
    $template = new TP\Legacy\Template(TEMPLATES_DIR . '/' . $tpl_dir_name);
...
function bb_die($msg_text)
{
    ...
    if (!defined('PAGE_HEADER_SENT')) {
        if (empty($template)) {
            $template = new TP\Legacy\Template(BB_ROOT . "templates/{$bb_cfg['tpl_name']}");
        }
        if (empty($theme)) {
            $theme = setup_style();
        }
...
function clean_text_match($text, $ltrim_star = true, $die_if_empty = false)
{
    ...
    if ($bb_cfg['search_engine_type'] == 'sphinx') {
        ...
        $text_match_sql = OLD_DB()->escape(trim($text));
    }
...
function send_pm($user_id, $subject, $message, $poster_id = BOT_UID)
{
    ...
    $subject = OLD_DB()->escape($subject);
    $message = OLD_DB()->escape($message);
