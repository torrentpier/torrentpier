...
<!-- IF CFG_USE_AP && (AUTH_DELETE || AUTH_REPLY || AUTH_EDIT) -->
<script type="text/javascript">
...
<div id="poll-edit-tpl" style="display: none;">
	...
	<legend id="poll-legend"></legend>
		...
		<div class="med mrg_4"><i>{L_NEW_POLL_M_EXPLAIN}: {CFG_MAX_PO})</i></div>
	...
	<td class="message td2" rowspan="2">
		...
		<div class="post_head">
			...
			<p style="float: right; padding: 3px 2px 4px;" class="post_btn_1">
				...
				<!-- IF postrow.QUOTE --><a class="txtb" href="<!-- IF CFG_USE_AP -->" onclick="ajax.exec({ action: 'posts', post_id: {postrow.POST_ID}, type: 'reply'}); return false;<!-- ELSE -->{QUOTE_URL}{postrow.POST_ID}<!-- ENDIF -->">{QUOTE_IMG}</a>{POST_BTN_SPACER}<!-- ENDIF -->
				<!-- IF postrow.EDIT --><a class="txtb" href="<!-- IF CFG_USE_AP -->" onclick="edit_post({postrow.POST_ID}, 'edit'); return false;<!-- ELSE -->{EDIT_POST_URL}{postrow.POST_ID}<!-- ENDIF -->">{EDIT_POST_IMG}</a>{POST_BTN_SPACER}<!-- ENDIF -->
				<!-- IF postrow.DELETE --><a class="txtb" href="<!-- IF CFG_USE_AP -->" onclick="ajax.exec({ action: 'posts', post_id: {postrow.POST_ID}, topic_id : {TOPIC_ID}, type: 'delete'}); return false;<!-- ELSE -->{DELETE_POST_URL}{postrow.POST_ID}<!-- ENDIF -->">{DELETE_POST_IMG}</a>{POST_BTN_SPACER}<!-- ENDIF -->
