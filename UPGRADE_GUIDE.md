...
- [HTTP Request System Migration](#http-request-system-migration)
- [HTTP Helper Migration](#http-helper-migration)
- [Application Container & Architecture Migration](#-application-container--architecture-migration-1)
...
- Direct `$_GET`/`$_POST`/`$_REQUEST` access → Use `request()` methods
- Direct `$_COOKIE`/`$_SERVER`/`$_FILES` access → Use `request()` properties
...
**Important**: Direct script access (e.g., `/viewtopic.php`) is redirected to clean URLs by the web server.
